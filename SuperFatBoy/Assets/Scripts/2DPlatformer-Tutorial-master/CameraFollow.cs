...
public class CameraFollow : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (CrossPlatformInputManager.GetButton("Scale"))
        {
            ...
            {
                background.localScale = new Vector2(orthographicSize * 2.6f, orthographicSize * 2.4f);
                orthographicSize += cameraScaleSpeed * Time.deltaTime;//2.4-y, 2.6-x для увеличения бекграунда
            }
	...
	void LateUpdate() {
        ...
        focusArea.Update(target.bounds, camera.orthographicSize * camera.aspect - target.bounds.size.x, camera.orthographicSize - target.bounds.size.y);
        ...
        transform.position = CameraShift((Vector3)focusPosition + Vector3.forward * -10);
	}
